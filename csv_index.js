...
function getSKUData(paymentList, costOfGods, ads_brand, ads_display, ads_product, storageFee, surChargeFee, adjustment,
  ...
  paymentList.forEach(payment => {
    if (new Date("02/01/2023") <= new Date(payment.date.includes("PDT") ? payment.date.replace(" PDT", "") : payment.date.replace(" PST", "")) &&
      new Date(payment.date.includes("PDT") ? payment.date.replace(" PDT", "") : payment.date.replace(" PST", "")) < new Date("03/01/2023")) {
  ...
  adjustment.forEach(a => {
    var startDate = parseDate("02/01/2023")
    var endDate = parseDate("02/28/2023")
...
let splitEmptySku = async (data, payments) => {
  ...
  let filterPayments = payments.filter(p => (p.sku == '' && (new Date(p.date) >= new Date("02/01/2023")) 
  && (new Date(p.date) < new Date("02/28/2023"))));
  let count =0;
...
let findRemainFields = async (splitEmptySkuData, payments) => {
  splitEmptySkuData.forEach(sku => {
    ...
    (new Date(p.date) >= new Date("02/01/2023")) && (new Date(p.date) < new Date("02/28/2023")))
