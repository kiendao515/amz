...
function getSKUData(paymentList, costOfGods, ads_brand, ads_display, ads_product, storageFee, surChargeFee, adjustment,
  ...
  paymentList.forEach(payment => {
    if (new Date("11/01/2023") <= new Date(payment.date.includes("PDT") ? payment.date.replace(" PDT", "") : payment.date.replace(" PST", "")) &&
      new Date(payment.date.includes("PDT") ? payment.date.replace(" PDT", "") : payment.date.replace(" PST", "")) < new Date("12/01/2023")) {
  ...
  adjustment.forEach(a => {
    var startDate = parseDate("11/01/2023")
    var endDate = parseDate("12/01/2023")
...
let findCogs = async (rs, cogs_data) => {
  rs.forEach(sku => {
    let skuData = cogs_data.filter(t => t.sku === sku.sku && (new Date(t.date) >= new Date("11/01/2023")) && (new Date(t.date) < new Date("12/01/2023")) && t.disposition == "SELLABLE")
    if (skuData.length > 0) {
      sku.cogs_shipped = 0;
      sku.cogs_removal = 0;
      sku.cogs_adjusted = 0;
...
let splitEmptySku = async (data, payments) => {
  ...
  let filterPayments = payments.filter(p => (p.sku == '' && (new Date(p.date) >= new Date("2023-11-01T00:00:00-07:00"))
    && (new Date(p.date) < new Date("2023-12-01T00:00:00-07:00"))));
  let count = 0;
...
let findRemainFields = async (splitEmptySkuData, payments) => {
  splitEmptySkuData.forEach(sku => {
    ...
    let tmp = payments.filter(p => p.sku == sku.sku && p.description == "FBA Inventory Reimbursement - Lost:Inbound" &&
      (new Date(p.date) >= new Date("11/01/2023")) && (new Date(p.date) < new Date("12/01/2023")))
...
let GenerateFile = async () => {
  ...
  const payment_csv = './input/2023/T11/Payment T11.23.csv';
  const storage_csv = './input/2023/T11/Storage Fee T11.23.csv';
  const surcharge_csv = './input/2023/T11/Surcharge Fee T11.23.csv';
  const removal_csv = "./input/2023/T11/Removal Fee T11.23.csv"
  const inventory_ledger_csv = "./input/2023/T11/Inventory Ledger T11.23.csv"

  // handle ads
  const wb_ads_product = XLSX.readFile('./input/2023/T11/Sponsored Products Advertised product report T11.23.xlsx')
  const wb_ads_display = XLSX.readFile('./input/2023/T11/Sponsored Display Advertised product report T11.23.xlsx')
  const wb_ads_brand = XLSX.readFile('./input/2023/T11/Sponsored Brands Keyword report T11.23.xlsx')
  let ads_product = []
